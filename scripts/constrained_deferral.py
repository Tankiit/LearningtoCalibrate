...
def run_constrained_deferral(
    ...
    try:
        ...
        out["expected_error"] = {
            "error": str(exc),
            "max_expected_coverage_at_target": max_feasible_coverage(
                p_pred_cal,
                target_accuracy=1.0 - budgets.alpha,
            ),
        }
        if verbose:
            print(f"Expected LP infeasible: {exc}")
            print(
                "  max expected coverage at target accuracy "
                f"{1.0 - budgets.alpha:.3f}: "
                f"{out['expected_error']['max_expected_coverage_at_target']:.4f}"
            )
        return out
...
def run_bootstrap_features(
    ...
    if "expert_reliable" in h_blob:
        expert_reliable = np.asarray(h_blob["expert_reliable"], dtype=np.int64)
    ...
    if item_out is not None:
        expected = results.get("expected", {}).get("test")
        ...
        if expected is not None:
            item_df["policy_expected"] = expected["policy"]
            item_df["action_expected"] = expected["actions"]
            item_df["commit_score_expected"] = expected["commit_score"]
        else:
            item_df["policy_expected"] = np.nan
            item_df["action_expected"] = np.nan
            item_df["commit_score_expected"] = np.nan
