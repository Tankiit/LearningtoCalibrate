...
from eval.risk_coverage import aurc, risk_coverage_curve, coverage_at_acc
...
def train_variant_probes(
    ...
    cfg = OVATrainConfig(lr=lr, epochs=epochs, batch_size=batch_size)
    ...
    for variant_name, builder in VARIANTS.items():
        for s in audit_seeds:
            ...
            for method, s_arr in scores.items():
                ...
                c80 = coverage_at_acc(
                    scores=s_arr[:len(y_correct)], correct=y_correct,
                    target_acc=0.80,
                )
                c90 = coverage_at_acc(
                    scores=s_arr[:len(y_correct)], correct=y_correct,
