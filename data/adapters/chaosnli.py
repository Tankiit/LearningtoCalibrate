...
SOURCES = [
    JsonlSource(
        "https://raw.githubusercontent.com/easonnie/ChaosNLI/master/chaos_nli/data/chaosNLI_v1.0/chaosNLI_mnli_m.jsonl",
        split="mnli_m",
    ),
    JsonlSource(
        "https://raw.githubusercontent.com/easonnie/ChaosNLI/master/chaos_nli/data/chaosNLI_v1.0/chaosNLI_snli.jsonl",
        split="snli",
    ),
]
...
def load(max_rows: int = DEFAULT_MAX_ROWS) -> list[Record]:
    out: list[Record] = []
    env_dir = os.environ.get(LOCAL_DIR_ENV)
    local_dir = Path(env_dir) if env_dir else (
        MODAL_LOCAL_DIR if MODAL_LOCAL_DIR.exists() else DEFAULT_LOCAL_DIR
    )
    if local_dir.exists():
