...
MODEL_CONFIGS = {
    ...
    "qwen2_5_7b": {
        ...
        "n_probe_layers": 8,
        "token_mode":     "first",
        "description":    "Qwen2.5-7B-Instruct — 28 layers, 1× A100-40GB, ~$4",
    },
    "gemma2_9b": {
...
def _extract_one(model, tokenizer, device, question, answer, n_layers, token_mode):
    ...
    # Tokenizers for Llama/Mistral/Qwen can merge or absorb very short answers
    # ("yes", "D") when comparing tokenized prefix vs tokenized prefix+answer.
    # Build the sequence from explicit prompt ids plus explicit answer ids so
    # the answer span is stable across tokenizers and short medical labels.
    pfx = f"Question: {question}\nAnswer:"
    pids = tokenizer(pfx, add_special_tokens=True).input_ids
    aids = tokenizer(f" {answer}", add_special_tokens=False).input_ids
    if not aids:
        aids = tokenizer(str(answer), add_special_tokens=False).input_ids
    if not aids:
        return None, None
    a_s, a_e = len(pids), len(pids) + len(aids)
    fids = torch.tensor([pids + aids], dtype=torch.long, device=device)
    with torch.no_grad():
    ...
    answer_ids = fids[0, a_s:a_e]
    gen_lp = lp[a_s-1:a_e-1][torch.arange(len(answer_ids)), answer_ids].mean().item()
    return h, gen_lp
...
@app.function(image=image, gpu="A100-40GB", volumes=_VOLUMES,
              timeout=28800, memory=65536, secrets=_SECRETS)
def extract_qwen2_5_7b(datasets: list = None):
    _run_model("qwen2_5_7b", datasets or ALL_DATASETS)
